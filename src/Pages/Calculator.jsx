...
function Header() {
    return (
        <div className="grid grid-cols-6 text-center items-center bg-neutral-300 dark:bg-neutral-700 py-2 mb-2 rounded-lg">
            <h1 className="col-span-1">S.No</h1>
            <h1 className="col-span-2">Subject</h1>
            <h1 className="col-span-1">Credits</h1>
            <h1 className="col-span-2">Grade</h1>
        </div>
    );
}
    ...
    return (
      <div className="dropdown-menu flex flex-col justify-center items-center md:flex-row w-full md:w-auto">
        <select
          ...
          value={grade}
          className="dropdown-toggle rounded-md focus:outline-neutral-500 appearance-none cursor-pointer hover:bg-neutral-300 focus:bg-neutral-300 dark:hover:bg-neutral-700 dark:focus:bg-neutral-700 dark:focus:outline-neutral-600 py-1.5 px-4 w-11/12 md:w-auto text-center hover:shadow-lg hover:scale-[1.01] hover:transition-all duration-300 ease-in-out"
        >
...
function CreditDrop({ onCreditChange, reset, resetCredit }) {
    ...
    return (
      <div className="flex flex-col justify-center items-center md:flex-row w-auto md:w-auto">
        <select
          onChange={handleCreditChange}
          value={credit}
          className="rounded-md focus:outline-neutral-500 appearance-none cursor-pointer hover:bg-neutral-300 focus:bg-neutral-300 dark:hover:bg-neutral-700 dark:focus:bg-neutral-700 dark:focus:outline-neutral-600 py-1.5 px-4 w-11/12 md:w-auto text-center hover:shadow-lg hover:scale-[1.01] hover:transition-all duration-300 ease-in-out"
        >
          <option value="" id="text" disabled hidden>Credits</option>
...
function SubjectInput({ onSubjectChange, reset, resetSubject }) {
    ...
    return (
      <div>
        <input
          type="text"
          className="rounded-md py-1.5 px-4 w-full outline-none hover:bg-neutral-200 hover:cursor-pointer focus:cursor-text focus:bg-neutral-200 focus:outline-neutral-400 dark:hover:bg-neutral-500 dark:focus:bg-neutral-500 dark:focus:outline-neutral-600 md:w-auto text-center hover:shadow-lg hover:scale-[1.01] hover:transition-all duration-300 ease-in-out"
          placeholder="Subject"
          value={subject}
...
function CenteredHr() {
    return (
        <div className="flex justify-center">
            <hr className="my-2 w-4/6 border-neutral-500 opacity-50" />
        </div>
    );
}
...
function YearSem({ onYearChange, onSemesterChange, reset }) {
    ...
    return (
      <div className="grid grid-cols-4 text-center">
        <div></div>
        <div className="year row-span-1">
          <select
            className="rounded-md appearance-none cursor-pointer focus:outline-zinc-500  hover:bg-zinc-300 focus:bg-zinc-300 dark:focus:outline-zinc-400 dark:hover:bg-zinc-700 dark:focus:bg-zinc-700 py-1.5 px-4 w-11/12 md:w-auto text-center hover:shadow-lg hover:scale-[1.01] hover:transition-all duration-300 ease-in-out"
            value={selectedYear}
            onChange={handleYearChange}
          >
            <option value="" id="text" disabled hidden>Year</option>
        ...
        <div className="row-span-1">
          <select
            className="rounded-md focus:outline-zinc-500 appearance-none cursor-pointer hover:bg-zinc-300 focus:bg-zinc-300 dark:focus:outline-zinc-400 dark:hover:bg-zinc-700 dark:focus:bg-zinc-700 py-1.5 px-4 w-11/12 md:w-auto text-center hover:shadow-lg hover:scale-[1.01] hover:transition-all duration-300 ease-in-out"
            value={selectedSemester}
            onChange={handleSemesterChange}
            disabled={!selectedYear}
          >
            <option value="" id="text" disabled hidden>Semester</option>
  ...
  return new Promise((resolve) => {
      ...
      overlay.innerHTML = `
          <div class="bg-white p-6 rounded-lg shadow-lg text-center">
              <p class="mb-4 text-lg">Would you like to log in and save your data?</p>
              <button id="loginBtn" class="bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 px-4 rounded mr-2">
                  Log In
              </button>
              <button id="continueBtn" class="bg-gray-300 hover:bg-gray-400 text-gray-800 font-bold py-2 px-4 rounded">
...
function showGPAOverlay(gpa) {
  ...
  overlay.innerHTML = `
      <div class="bg-white p-6 rounded-lg shadow-lg text-center">
          <p class="mb-4 text-xl font-bold">Your GPA is: ${gpa}</p>
          <button id="closeBtn" class="bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 px-4 rounded">
              Close
          </button>
      </div>
  `;
...
function showErrorOverlay(message) {
  ...
  overlay.innerHTML = `
      <div class="bg-white p-6 rounded-lg shadow-lg text-center">
          <p class="mb-4 text-xl font-bold text-red-500">Error: ${message}</p>
          <button id="closeBtn" class="bg-red-500 hover:bg-red-600 text-white font-bold py-2 px-4 rounded">
              Close
          </button>
      </div>
  `;
...
export default function Calculator() {
    ...
    return (
      <div className="grid grid-rows-[10%_8%_71%_11%] w-11/12 h-[90%] py-2 rounded-lg">
